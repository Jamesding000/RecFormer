...
def eval(model, dataloader, args, save_pred_path):
    model.eval()
    ...
    for batch, labels in pbar:
        ...
        res = ranker(scores, labels, save_pred_path)
...
def main():
    ...
    save_pred_path = 'preds/Recformer_Video_Games.csv'
    os.makedirs(os.path.dirname(save_pred_path), exist_ok=True)
    header = "label,preds\n"
    # Open the file in 'w' mode and write the header
    ...
    model.load_state_dict(torch.load(path_ckpt))
    test_in_sample_metrics = eval(model, test_in_sample_loader, args, save_pred_path)
    print(f'Test set: {test_in_sample_metrics}')
        
    # test_cold_start_metrics = eval(model, test_cold_start_loader, args, save_pred_path)
